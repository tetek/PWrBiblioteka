...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.opacity = 0.8f;
        self.labelFont = [UIFont boldSystemFontOfSize:PH_LABELFONTSIZE];
        self.detailsLabelFont = [UIFont boldSystemFontOfSize:PH_LABELDETAILSFONTSIZE];
        self.xOffset = 0.0f;
        self.yOffset = 0.0f;
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        self.height = self.height + lHeight + PH_PADDING;
		
        // Move indicator to make room for the label
        indFrame.origin.y -= (floorf(lHeight / 2 + PH_PADDING / 2));
        indicator.frame = indFrame;
		
        // Set the label position and dimensions
        CGRect lFrame = CGRectMake(floorf((frame.size.width - lWidth) / 2) + xOffset,
                                   floorf(indFrame.origin.y + indFrame.size.height + PH_PADDING),
                                   lWidth, lHeight);
        label.frame = lFrame;
        ...
        if (nil != self.detailsLabelText) {
            ...
            lFrame.origin.y -= (floorf(lHeight / 2 + PH_PADDING / 2));
            label.frame = lFrame;
			
            // Set label position and dimensions
            CGRect lFrameD = CGRectMake(floorf((frame.size.width - lWidth) / 2) + xOffset,
                                        lFrame.origin.y + lFrame.size.height + PH_PADDING, lWidth, lHeight);
